...
export class ClientFormComponent implements OnInit {
  // Helper for template
  getClientName(client: Client | any): string {
    if (!client) return '';
    if (client.typeClient === 'professionnel' || client.raisonSociale) {
  ...
  ngOnInit(): void {
    ...
    // Setup parrain search
    this.parrainSearchCtrl.valueChanges.pipe(
      debounceTime(300),
      distinctUntilChanged(),
      filter(value => typeof value === 'string' && value.length >= 2),
      switchMap(query => this.clientService.searchClientsByNom(query || ''))
    ).subscribe(clients => {
      this.filteredParrains.set(clients);
    });
  }
  ...
  private openFamilyCheckDialog(existingClients: Client[], currentNom: string, isSpouseSearch: boolean = false): void {
    ...
    if (isSpouseSearch) {
      (dialogRef.componentInstance as any).selectedLienParental = 'Conjoint';
    }
  ...
  displayParrain(client: any): string {
    if (!client) return '';
    return client.nom ? `${client.nom} ${client.prenom || ''}` : (client.raisonSociale || '');
  }

  onParrainSelected(event: any): void {
    const client = event.option.value;
    this.clientForm.get('parrainId')?.setValue(client.id);
  }
  ...
  private populateForm(client: Client): void {
    ...
    if (client.typeClient === TypeClient.PARTICULIER) {
      ...
      if (client.parrainId) {
        this.clientService.getClient(client.parrainId).subscribe(parrain => {
          if (parrain) {
            this.parrainSearchCtrl.setValue(parrain as any, { emitEvent: false });
          }
        });
      }
  ...
  private buildClientData(): any {
    ...
    if (typeClient === TypeClient.PARTICULIER) {
      return {
        ...
        parrainId: formValue.parrainId || undefined
      };
  ...
  private createClient(clientData: any): void {
    this.clientService.createClient(clientData).subscribe({
      ...
      error: (error) => {
        ...
        const errorMsg = `Erreur ${error.status}: ${serverMessage}`;
        alert(errorMsg);
