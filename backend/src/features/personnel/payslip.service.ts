...
export class PayslipService {
  ...
  async generate(
    ...
    doc.font('Helvetica');
    doc.text(
      `Nom : ${employee.nom.toUpperCase()} ${employee.prenom}`,
      50,
      infoY + 15,
    );
    ...
    doc.text(
      `Date Embauche : ${new Date(employee.dateEmbauche).toLocaleDateString()}`,
      300,
      infoY + 15,
    );
    doc.text(`Type contrat : ${employee.contrat}`, 300, infoY + 30);
    doc.text(
      `Mode Paiement : ${employee.paymentMode || 'VIREMENT'}`,
      300,
      infoY + 45,
    );
    doc.moveDown(3);
    ...
    if (payroll.healthInsuranceDeduction > 0) {
      ...
      doc.text(
        `${payroll.healthInsuranceDeduction.toFixed(2)}`,
        460,
        currentY,
    ...
    doc.text(
      `Base de calcul Net Imposable : ${(payroll.taxableNet || 0).toFixed(2)} MAD`,
      50,
      doc.y,
    );
