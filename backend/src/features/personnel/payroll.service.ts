...
export class PayrollService {
  ...
  async generate(dto: GeneratePayrollDto) {
    ...
    if (existing && existing.statut !== 'BROUILLON') {
      throw new BadRequestException(
        'Un bulletin validé ou payé existe déjà pour ce mois',
      );
    }
    ...
    const attendanceStats = await this.attendanceService.getStats(
      employeeId,
      mois,
      annee,
    );
  ...
  private calculatePayrollInternal(
    employee: any,
    inputs: {
      salaireBase: number;
      commissions: number;
    ...
    const taxableBaseForProfExpenses =
      grossSalary - socialSecurityDeduction - healthInsuranceDeduction;
    ...
    if (config.profExpensesThreshold && config.profExpensesRateLow !== null) {
      ...
      const rate = isLowIncome
        ? config.profExpensesRateLow
        : config.profExpensesRateHigh;
      const annualCap = isLowIncome
        ? config.profExpensesCapLow
        : config.profExpensesCapHigh;
    ...
    const brackets = Array.isArray(config.incomeTaxBrackets)
      ? config.incomeTaxBrackets
      : [];
    const bracket = brackets.find(
      (b: any) =>
        taxableNet >= b.min && (b.max === null || taxableNet <= b.max),
    );

    if (bracket) {
      incomeTaxDeduction =
        (taxableNet * bracket.rate) / 100 - (bracket.deduction || 0);
    }
    ...
    const dependentCount =
      ...
      (employee.familyStatus === 'MARIE' ? 1 : 0);
    ...
    const employerAF =
      (grossSalary * (config.familyAllowanceRate_P || 0)) / 100;
    ...
    const employerCharges =
      employerSS + employerAF + employerTFP + employerHealth;
  ...
  async validate(id: string) {
    ...
    const pdfUrl = (await this.payslipService.generate(
      payroll.employee,
      payroll,
      commissions,
    )) as string;
  ...
  async getGeneratedPdf(id: string) {
    try {
      ...
      console.log(
        `[PayrollService] Found payroll for ${payroll.employee?.nom}. Fetching commissions...`,
      );
      const commissions = await this.commissionService.getEmployeeCommissions(
        payroll.employeeId,
        payroll.mois,
      ...
      console.log(
        `[PayrollService] Commissions count: ${commissions.length}. Calling payslipService.generate...`,
      );
      // Force re-generation
      const pdfUrl = await this.payslipService.generate(
        payroll.employee,
      ...
      console.error(
        `[PayrollService] Error generating PDF for payroll ${id}:`,
        error,
      );
      throw error;
    }
  ...
  async markAsPaid(
    ...
    if (payroll.statut === 'PAYE')
      throw new BadRequestException('Bulletin déjà payé');
  ...
  async getDashboardStats(mois?: string, annee?: number, centreId?: string) {
    ...
    const totalCommissions = payrolls.reduce(
      (sum, p) => sum + p.commissions,
      0,
    );
    const totalEmployerCharges = payrolls.reduce(
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      history.push({
        ...
        totalMass: hPayrolls.reduce(
          (sum, p) => sum + p.netAPayer + (p.employerCharges || 0),
          0,
        ),
      });
    }
    ...
    return {
      ...
      employeeCount,
      history,
      posteDistribution: Object.entries(posteDistribution).map(
