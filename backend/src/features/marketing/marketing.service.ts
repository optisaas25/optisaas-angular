...
export class MarketingService {
      ...
      try {
        ...
        if (channel === 'WHATSAPP' && formattedPhone) {
          ...
          await this.sendEmail(
            (client as any).email,
            `Offre Spéciale: ${promoName || 'Votre promotion'}`,
            message,
            config,
          );
        } else {
          this.logger.warn(
            `[CAMPAIGN] Skipping client ${client.id} - No valid ${channel} contact info`,
          );
          continue;
        }
        ...
        this.logger.error(
          `[CAMPAIGN] Failed to send to ${client.id} via ${channel}: ${error.message}`,
        );
  ...
  private async sendWhatsApp(phone: string, message: string, dbConfig?: any) {
    ...
    try {
      ...
      this.logger.log(
        `[WhatsApp] Send successful for ${phone}: ${JSON.stringify(response.data)}`,
      );
    } catch (error) {
      this.logger.error(
        `[WhatsApp] Failed to send to ${phone}: ${error.response?.data ? JSON.stringify(error.response.data) : error.message}`,
      );
      throw error;
    }
  }
  ...
  private async sendSMS(phone: string, message: string, dbConfig?: any) {
    const apiUrl =
      dbConfig?.smsGatewayUrl ||
      this.configService.get<string>('SMS_GATEWAY_URL');
    const apiKey =
      dbConfig?.smsApiKey || this.configService.get<string>('SMS_API_KEY');
    ...
    try {
      ...
      this.logger.log(
        `[SMS] Send successful for ${phone}: ${JSON.stringify(response.data)}`,
      );
    } catch (error) {
      this.logger.error(
  ...
  private async sendEmail(
    ...
    const host =
      dbConfig?.smtpHost || this.configService.get<string>('SMTP_HOST');
    const user =
      dbConfig?.smtpUser || this.configService.get<string>('SMTP_USER');
    const pass =
      dbConfig?.smtpPass || this.configService.get<string>('SMTP_PASS');
    const port =
      Number(dbConfig?.smtpPort) ||
      this.configService.get<number>('SMTP_PORT', 587);

    this.logger.log(
      `[Email] Attempting send to ${to} via ${host}:${port} (user: ${user})`,
    );
