...
export class ProductsService {
      ...
      return await this.prisma.$transaction(async (tx) => {
        const product = await tx.product.create({
          data: {
            ...
            utilisateurCreation:
              mainAndRelationalFields.utilisateurCreation || 'system',
          },
        });
  ...
  async remove(id: string) {
    ...
    if (product.quantiteActuelle > 0) {
      throw new Error(
        `Suppression impossible : ce produit possède encore du stock (${product.quantiteActuelle}). Veuillez d'abord sortir le stock manuellement.`,
      );
    }
    ...
    if (product._count.mouvements > 0) {
      throw new Error(
        `Suppression impossible : ce produit possède un historique de mouvements de stock (${product._count.mouvements}). Pour la traçabilité, vous ne pouvez pas le supprimer.`,
      );
    }
    ...
    if (linkedInvoice) {
      throw new Error(
        `Suppression impossible : ce produit est référencé dans la facture ${linkedInvoice.numero}.`,
      );
    }
  ...
  async initiateTransfer(
    ...
    if (sourceProductId === targetProductId)
      throw new Error(
        'Impossible de transférer un produit vers lui-même (Source = Destination).',
      );
    if (sourceProduct.quantiteActuelle < quantite)
      throw new Error('Stock insuffisant à la source');

    console.log(
      `[TRANSFER] Init from ${sourceProduct.entrepot?.centre?.nom} (${sourceProductId}) to ${targetProduct.entrepot?.centre?.nom} (${targetProductId}) Qty: ${quantite}`,
    );
      ...
      if (sourceProductId) {
        ...
        if (sourceProduct) {
          ...
          if (ssd.pendingOutgoing) {
            const outgoing = ssd.pendingOutgoing.find(
              (t: any) => t.targetProductId === targetProductId,
            );
  ...
  async bulkShip(targetProductIds: string[]) {
    ...
    if (!targetProductIds || targetProductIds.length === 0) {
      throw new BadRequestException(
        "Aucun produit sélectionné pour l'expédition",
      );
    }
    ...
    console.log(
      `[BULK-SHIP] Complete. Success: ${results.length}, Failures: ${errors.length}`,
    );
  ...
  async bulkReceive(targetProductIds: string[]) {
    ...
    if (!targetProductIds || targetProductIds.length === 0) {
      throw new BadRequestException(
        'Aucun produit sélectionné pour la réception',
      );
    }
    ...
    for (const id of targetProductIds) {
      try {
        ...
        console.error(
          `[BULK-RECEIVE] ❌ Error receiving ${id}:`,
          error.message,
        );
        errors.push({ id, error: error.message });
      }
    ...
    console.log(
      `[BULK-RECEIVE] Complete. Success: ${results.length}, Failures: ${errors.length}`,
    );
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (sourceProduct) {
        ...
        if (sourceSd.pendingOutgoing) {
          sourceSd.pendingOutgoing = sourceSd.pendingOutgoing.filter(
            (t: any) => t.targetProductId !== targetProductId,
          );
          await tx.product.update({
            where: { id: sourceProductId },
      ...
      const newStatus =
        targetProduct.quantiteActuelle > 0 ? 'DISPONIBLE' : 'RUPTURE';
  ...
  async completeTransfer(targetProductId: string) {
    ...
    if (sd.pendingIncoming.status !== 'SHIPPED') {
      throw new Error(
        "Le transfert doit être expédié par la source avant d'être reçu.",
      );
    }
  ...
  async restock(
    ...
    try {
      ...
      const cleanNewPrice =
        prixAchatHT !== undefined && !isNaN(Number(prixAchatHT))
          ? Number(prixAchatHT)
          : undefined;
      const cleanRemise =
        remiseFournisseur !== undefined && !isNaN(Number(remiseFournisseur))
      ...
      if (!product.entrepotId)
        throw new BadRequestException(
          "Le produit n'est rattaché à aucun entrepôt",
        );

      // Apply discount to the new price if provided
      const finalNewPriceHT =
        cleanNewPrice !== undefined
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (finalNewPriceHT !== undefined) {
          const currentStock =
            product.quantiteActuelle > 0 ? product.quantiteActuelle : 0;
  ...
  async destock(
    ...
    try {
      ...
      if (product.quantiteActuelle < cleanQuantite) {
        throw new Error(
          `Quantité insuffisante en stock (Actuel: ${product.quantiteActuelle})`,
        );
      }
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        const isDefective =
          motif?.toLowerCase().includes('casse') ||
        ...
        const updatedProduct = await tx.product.update({
          ...
          data: {
            ...
            statut:
              product.quantiteActuelle - cleanQuantite <= 0
                ? 'RUPTURE'
                : product.statut,
          },
        });
        ...
        if (effectiveDestinationId) {
          ...
          console.log(
            `🚚 [DESTOCK-DEBUG] Transfer requested. Dest: ${effectiveDestinationId} (${destinationWh?.nom})`,
          );
          console.log(
            `   📍 Source Center: ${product.entrepot.centreId} | Dest Center: ${destinationWh?.centreId}`,
          );
          ...
          if (isCrossCenter) {
            ...
            console.log(
              `   💾 Updating Source Product ${updatedProduct.id} with pendingOutgoing`,
              updatedSsd.pendingOutgoing,
            );
            ...
            console.log(`   🎯 [DESTOCK-DEBUG] Target Product AFTER update:`, {
              ...
              pendingIncomingStatus:
                targetProduct.specificData?.pendingIncoming?.status,
            });
            ...
            console.log(
              `   ✅ [DESTOCK-VERIFY] Saved Source Product ${savedProduct.id}. pendingOutgoing length: ${savedProduct.specificData?.pendingOutgoing?.length}`,
            );
      ...
      if (
        ...
        throw error;
      throw new BadRequestException(
        error.message || 'Une erreur est survenue lors de la sortie de stock',
      );
    }
  }
  ...
  async syncProductState(productId: string, tx?: any) {
    ...
    const newQty = product.mouvements.reduce(
      (sum, mov) => sum + mov.quantite,
      0,
    );
    ...
    for (const product of productsInRupture) {
      ...
      try {
        ...
        try {
          ...
          console.error(
            `Failed to archive product ${product.id}:`,
            archiveError,
          );
        }
      }
