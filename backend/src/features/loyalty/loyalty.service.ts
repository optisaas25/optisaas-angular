...
export class LoyaltyService {
  ...
  async recalculateAllClientsPoints() {
    setImmediate(async () => {
      try {
        console.log('[LOYALTY-ASYNC] Starting background global recalculation of loyalty points...');
        const config = await this.getConfig();
        ...
        const clients = await this.prisma.client.findMany({
          include: {
            ...
            factures: { select: { id: true, numero: true, totalTTC: true, type: true, proprietes: true } },
          },
        });
        console.log('[LOYALTY-ASYNC] Recalculating points for ' + clients.length + ' clients...');
        await this.prisma.pointsHistory.deleteMany({
          where: {
            OR: [{ type: 'FOLDER_CREATION' }, { type: 'EARN' }, { type: 'SPEND', factureId: { not: null } }],
          },
        });
        ...
        for (let i = 0; i < clients.length; i += BATCH_SIZE) {
          ...
          console.log('[LOYALTY-ASYNC] Processed batch ' + batchNum + '/' + totalBatches);
        }
        console.log('[LOYALTY-ASYNC] Recalculation complete!');
      } catch (err) {
        console.error('[LOYALTY-ASYNC] Background recalculation failed:', err);
      }
    });
  }
