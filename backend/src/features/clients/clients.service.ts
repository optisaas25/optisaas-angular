    ...
    if (centreId) {
      ...
      if (!centreExists) {
        throw new BadRequestException(
          `Centre non trouvé (${centreId}). Votre session est peut-être obsolète. Veuillez vous déconnecter et vous reconnecter pour rafraîchir vos accès.`,
        );
      }
    }
  ...
  async remove(id: string) {
    ...
    if (clientValidInvoices.length > 0) {
      ...
      const isSafeToDelete =
        clientIds.length === globalIds.length &&
        clientIds.every((val, index) => val === globalIds[index]);

      if (!isSafeToDelete) {
        const globalLast = globalLastValidInvoices[0];
        throw new Error(
          `Supression impossible: La continuité fiscale n'est pas respectée. Une facture plus récente existe (${globalLast?.numero}). Action strictement interdite.`,
        );
      }
    }
    ...
    if (openFichesCount > 0) {
      throw new Error(
        "Supression impossible: Ce client a des fiches en cours. Veuillez les annuler d'abord.",
      );
    }
