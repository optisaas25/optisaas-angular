...
export class JourneeCaisseService {
  ...
  async ouvrir(ouvrirCaisseDto: OuvrirCaisseDto) {
    ...
    if (
      ouvrirCaisseDto.caissier.match(
  ...
  async cloturer(id: string, cloturerCaisseDto: CloturerCaisseDto) {
    ...
    const ecartCarteMontant =
      cloturerCaisseDto.montantTotalCarte - soldeTheoriqueCarte;
    const ecartChequeMontant =
      cloturerCaisseDto.montantTotalCheque - soldeTheoriqueCheque;

    // Individual écarts (Count)
    const ecartCarteNombre =
      cloturerCaisseDto.nbRecuCarte - stats.nbVentesCarte;
    const ecartChequeNombre =
      cloturerCaisseDto.nbRecuCheque - stats.nbVentesCheque;
    ...
    if (
      cloturerCaisseDto.responsableCloture.match(
        /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i,
      )
    ) {
      const user = await this.prisma.user.findUnique({
  ...
  async getActiveByCaisse(caisseId: string) {
    ...
    if (!journee) {
      throw new NotFoundException(
        'Aucune journée de caisse ouverte pour cette caisse',
      );
    }
  ...
  async getResume(id: string, startDate?: string, endDate?: string) {
    console.log(
      `[Caisse-Resume] getResume: ID=${id}, start=${startDate}, end=${endDate}`,
    );
