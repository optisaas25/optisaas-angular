...
export class MailerService {
  ...
  private getOrCreateTransporter(
    ...
    this.logger.log(
      `[Mailer] Creating new SMTP transporter for ${host}:${port}`,
    );
  ...
  async sendMailWithAttachment(options: {
    ...
    const host =
      config?.smtpHost || this.configService.get<string>('SMTP_HOST');
    const user =
      config?.smtpUser || this.configService.get<string>('SMTP_USER');
    const pass =
      config?.smtpPass || this.configService.get<string>('SMTP_PASS');
    const port =
    ...
    if (!host || !user || !pass) {
      this.logger.error(
        '[Mailer] SMTP credentials missing! Configure SMTP settings in Marketing Config.',
      );
    ...
    const fromEmail =
      config?.smtpFrom && config.smtpFrom.includes('@')
        ? config.smtpFrom
