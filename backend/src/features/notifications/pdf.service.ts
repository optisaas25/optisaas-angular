...
export class PdfService {
  ...
  async generatePurchaseOrder(data: {
    ...
    return new Promise((resolve, reject) => {
      console.log(
        '📄 [PdfService] >>> GENERATING PURCHASE ORDER PDF - VERSION 2.2 (FIXED AXE/DIAM) <<<',
      );
      ...
      doc
        ...
        .fontSize(24)
        .text(
          data.branding?.companyName?.toUpperCase() || 'OPTISASS',
          headerX,
          40,
          { align: hasLogo ? 'right' : 'left' },
        );
      doc
        .fillColor(accentColor)
        .font('Helvetica-Bold')
        .fontSize(16)
        .text('BON DE COMMANDE VERRES', headerX, 68, {
          align: hasLogo ? 'right' : 'left',
        });
      doc
        .fillColor(greyText)
        ...
        .fontSize(10)
        .text(
          `Date: ${data.date.toLocaleDateString('fr-FR')} | Fournisseur: ${data.supplierName}`,
          headerX,
          88,
          { align: hasLogo ? 'right' : 'left' },
        );
      ...
      drawCard(
        ...
        'Client & Dossier',
        data.clientName,
        `Fiche N°: ${data.ficheNumber || data.bcNumber.split('-').pop()}`,
      );
      drawCard(
        305,
        startY,
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(9)
        .text('CARACTÉRISTIQUES DES VERRES', 40, sectionY);
      doc
        .moveTo(40, sectionY + 12)
        .lineTo(555, sectionY + 12)
        .stroke('#f1f5f9');
      ...
      drawSpec(
        ...
        gridY,
        'Matière',
        data.lensDetails?.matiereOD === data.lensDetails?.matiereOG
          ? data.lensDetails?.matiereOD || '-'
          : `OD: ${data.lensDetails?.matiereOD} | OG: ${data.lensDetails?.matiereOG}`,
      );

      const formatDiam = (d: any) => (d && d !== '-' ? `${d} mm` : d || '-');

      const diamUtileLabel =
        data.prescription.od.diamUtile === data.prescription.og.diamUtile
      ...
      drawSpec(
        ...
        gridY + 45,
        'INDICE',
        data.lensDetails?.indiceOD === data.lensDetails?.indiceOG
          ? data.lensDetails?.indiceOD || '-'
          : `OD: ${data.lensDetails?.indiceOD} | OG: ${data.lensDetails?.indiceOG}`,
        240,
      );
      ...
      drawSpec(
        ...
        gridY + 90,
        'TRAITEMENTS',
        data.lensDetails?.treatments || 'STANDARD',
        240,
      );
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(9)
        .text('PRESCRIPTION TECHNIQUE', 40, tableTop - 15);
      ...
      const drawRow = (label: string, p: any, y: number) => {
        ...
        doc.text(p.sphere || '+0.00', 85, y + 10, {
          width: 80,
          align: 'center',
        });
        doc.text(p.cylindre || '0.00', 165, y + 10, {
          width: 80,
          align: 'center',
        });
        doc.text(axeStr, 245, y + 10, { width: 80, align: 'center' });
        doc.text(p.addition || '+0.00', 325, y + 10, {
          width: 80,
          align: 'center',
        });
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(10)
        .text('Cachet et Signature', 40, signY, {
          width: 515,
          align: 'center',
        });
      ...
      if (data.branding?.cachetUrl) {
        try {
          ...
          doc.image(Buffer.from(base64Cachet, 'base64'), 370, signY + 30, {
            width: 150,
          });
        } catch (e) {
      ...
      if (data.observations && data.observations.trim() !== '') {
        ...
        doc
          ...
          .font('Helvetica-Bold')
          .fontSize(9)
          .text('NOTES / REMARQUES', 55, obsY + 12);
        doc
          .fillColor(primaryColor)
          .font('Helvetica')
          .fontSize(10)
          .text(data.observations, 55, obsY + 28, {
            width: 485,
            align: 'left',
          });
      }
      ...
      doc
        .fontSize(8)
        .fillColor(greyText)
        .text('OptiSaas - Solution de gestion optique', 40, 785);
      doc.text(
  ...
  async generateFicheMontagePdf(data: {
    ...
    return new Promise((resolve, reject) => {
      console.log(
        '📄 [PdfService] >>> GENERATING FICHE MONTAGE PDF - VERSION 2.2 (FIXED AXE/DIAM/IMAGE) <<<',
      );
      const doc = new PDFDocument({ margin: 40, size: 'A4' });
      const chunks: Buffer[] = [];
      doc.on('data', (chunk: Buffer) => chunks.push(chunk));
      ...
      doc
        ...
        .fontSize(16)
        .text(
          data.branding?.companyName?.toUpperCase() || 'OPTISAAS',
          headerX,
          25,
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(11)
        .text('FICHE DE MONTAGE TECHNIQUE', headerX, 45, {
          align: 'right',
          width: 265,
        });
      doc
        ...
        .fontSize(8)
        .text(
          `REF: ${data.bcNumber} | LE: ${data.date.toLocaleDateString('fr-FR')}`,
          headerX,
          60,
          { align: 'right', width: 265 },
        );
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(9)
        .text('CLIENT: ', 45, rowY + 8, { continued: true })
        .font('Helvetica')
        .text(data.clientName);
      doc
        .font('Helvetica-Bold')
        .text('MAGASIN: ', 360, rowY + 8, { continued: true })
        .font('Helvetica')
        .text(data.magasinName || 'OptiSaas');
      ...
      const drawTechTable = (
        ...
        headers.forEach((h, i) => {
          ...
          doc
            ...
            .font('Helvetica-Bold')
            .fontSize(7)
            .text(h, curX, y + 22, { width: cw[i], align: 'center' });
          curX += cw[i];
        });
        ...
        rows.forEach((r, idx) => {
          ...
          doc
            ...
            .font('Helvetica-Bold')
            .fontSize(9)
            .text(r.label, curX, ry + 6, { width: cw[0], align: 'center' });
          curX += cw[0];
      ...
      drawTechTable(
        30,
        tablesY,
        ['OEIL', 'SPHÈRE', 'CYLINDRE', 'AXE', 'ADD'],
        [
          {
            label: 'OD',
            c1: data.prescription.od.sphere,
            c2: data.prescription.od.cylindre,
            c3: data.prescription.od.axe,
            c4: data.prescription.od.addition,
          },
          {
            label: 'OG',
            c1: data.prescription.og.sphere,
            c2: data.prescription.og.cylindre,
            c3: data.prescription.og.axe,
            c4: data.prescription.og.addition,
          },
        ],
      );
      ...
      const formatCentrageValue = (v: any) =>
        v && v !== '-' && !String(v).includes('mm') ? `${v} mm` : v || '-';
      drawCentrageTable(
        305,
        tablesY,
        ['OEIL', 'DP', 'HT (H)', 'DIAM. UTILE'],
        [
          {
            label: 'OD',
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(9)
        .text('TYPES DE VERRES SÉLECTIONNÉS', 40, lensY + 10);
      doc
        .fillColor(primaryColor)
        .fontSize(8)
        .font('Helvetica-Bold')
        .text('OD: ', 40, lensY + 25, { continued: true })
        .font('Helvetica')
        .text(data.verres.od);
      doc
        .font('Helvetica-Bold')
        .text('OG: ', 40, lensY + 40, { continued: true })
        .font('Helvetica')
        .text(data.verres.og);
      doc
        .fillColor(greyText)
        .fontSize(8)
        .font('Helvetica')
        .text(`TRAITEMENTS: `, 40, lensY + 55, { continued: true })
        .fillColor(greyText)
        .text(data.verres.treatments || 'Anti-lumière bleue (Blue Cut)');
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(8)
        .text('DIAMÈTRE À COMMANDER', 390, lensY + 20, {
          align: 'center',
          width: 165,
        });
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(26)
        .text(data.diametreConseille || '65/65', 380, lensY + 40, {
          width: 185,
          align: 'center',
        });
      ...
      if (data.technicalNote && data.technicalNote.ordered) {
        ...
        doc
          ...
          .font('Helvetica-Bold')
          .fontSize(8.5)
          .text(adviceText, 30, currentY + 6, { width: 535, align: 'center' });
        currentY += 30;
      }
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(9)
        .text('APERÇU CONFIGURATION MONTURE', 40, currentY + 10);

      if (data.virtualCenteringUrl) {
        try {
          const base64Centring = data.virtualCenteringUrl.includes('base64,')
            ? data.virtualCenteringUrl.split('base64,')[1]
            : data.virtualCenteringUrl;
          doc.image(Buffer.from(base64Centring, 'base64'), 132, currentY + 30, {
            fit: [300, 160],
            align: 'center',
            valign: 'center',
          });
          ...
          doc
            .fillColor(greyText)
            .fontSize(8)
            .text('Image centrage non disponible', 30, currentY + 100, {
              width: 535,
              align: 'center',
            });
        }
      ...
      const drawIABox = (x: number, y: number, side: string, val: string) => {
        ...
        doc
          ...
          .font('Helvetica')
          .fontSize(8)
          .text('Configuration patient: ', x + 10, y + 23, { continued: true })
          .fillColor(primaryColor)
          .text(val);
        ...
        doc
          ...
          .font('Helvetica-Bold')
          .fontSize(7)
          .text(side, x + 230, y + 24, { width: 20, align: 'center' });
      };
      ...
      if (data.observations) {
        ...
        doc
          ...
          .font('Helvetica-Bold')
          .fontSize(9)
          .text('NOTES / REMARQUES:', 30, obsY + 10);
        doc
          .font('Helvetica')
          .text(data.observations, 30, obsY + 25, { width: 535 });
        currentY += 40;
      }
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(8.5)
        .text('TECHNICIEN / MONTEUR', 30, footY + 10, {
          width: 230,
          align: 'center',
        });
      doc.text('CONTRÔLE FINAL', 305, footY + 10, {
        width: 260,
        align: 'center',
      });
      ...
      if (data.branding?.cachetUrl) {
        try {
          const base64Cachet = data.branding.cachetUrl.includes('base64,')
            ? data.branding.cachetUrl.split('base64,')[1]
            : data.branding.cachetUrl;
          doc.image(Buffer.from(base64Cachet, 'base64'), 400, footY + 30, {
            height: 45,
          });
        } catch (e) {
      ...
      doc
        .fontSize(7)
        .fillColor(greyText)
        .text('OptiSaas - Solution de gestion optique premium', 30, 790);
      doc.text(
        `Document généré le ${new Date().toLocaleString('fr-FR')}`,
        400,
        790,
  ...
  async generateLensPurchaseOrder(data: {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(14)
        .text('BON DE COMMANDE LENTILLES', headerX, 68, {
          align: hasLogo ? 'right' : 'left',
        });
      doc
        .fillColor(greyText)
        .font('Helvetica')
        .fontSize(10)
        .text(`Date: ${data.date.toLocaleDateString('fr-FR')}`, headerX, 85, {
          align: hasLogo ? 'right' : 'left',
        });
      ...
      drawCard(
        ...
        70,
        'Référence BC / Date',
        `${data.bcNumber} — ${data.date.toLocaleDateString('fr-FR')}`,
      );
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(9)
        .text('DÉTAILS DE LA COMMANDE', 40, sectionY);
      ...
      const drawRow = (label: string, p: any, y: number) => {
        ...
        doc
          ...
          .font('Helvetica-Bold')
          .fontSize(11)
          .text(label, 40, y + 12, { width: colWidths[0], align: 'center' });

        doc.fillColor(primaryColor).font('Helvetica').fontSize(10.5);
        doc.text(
          `${data.lensDetails.marque} ${data.lensDetails.modele}`,
          40 + colWidths[0],
          y + 12,
          { width: colWidths[1], align: 'left' },
        );
        doc.text(p.sphere || '0.00', 40 + colWidths[0] + colWidths[1], y + 12, {
          width: colWidths[2],
          align: 'center',
        });
        doc.text(
          `${p.cylindre || '0.00'} / ${p.axe || '0'}°`,
          40 + colWidths[0] + colWidths[1] + colWidths[2],
          y + 12,
          { width: colWidths[3], align: 'center' },
        );
        doc.text(
          `${p.rayon || '-'} / ${p.diametre || '-'}`,
          40 + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3],
          y + 12,
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(10)
        .text('Cachet du Magasin', 40, cachetY, {
          width: 515,
          align: 'center',
        });
      doc
        .rect(225, cachetY + 15, 150, 80)
        .dash(5, { space: 5 })
        .stroke('#cbd5e1');
      ...
      if (data.branding?.cachetUrl) {
        try {
          const base64Cachet = data.branding.cachetUrl.includes('base64,')
            ? data.branding.cachetUrl.split('base64,')[1]
            : data.branding.cachetUrl;
          doc.image(Buffer.from(base64Cachet, 'base64'), 250, cachetY + 20, {
            width: 100,
          });
        } catch (e) {
  ...
  async generateLensTechnicalSheet(data: {
    ...
    return new Promise((resolve, reject) => {
      console.log(
        '📄 [PdfService] >>> GENERATING LENS TECHNICAL SHEET PDF <<<',
      );
      const doc = new PDFDocument({ margin: 40, size: 'A4' });
      const chunks: Buffer[] = [];
      ...
      if (hasLogo && data.branding?.logoUrl) {
        try {
          ...
          this.logger.error(
            `Failed to embed lens tech sheet logo: ${e.message}`,
          );
        }
      }
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(13)
        .text('FICHE TECHNIQUE LENTILLES', headerX, 64, { align: 'right' });
      doc
        .fillColor(greyText)
        .font('Helvetica-Bold')
        .fontSize(8)
        .text(
          `RÉF: ${data.bcNumber} | LE: ${data.date.toLocaleDateString('fr-FR')}`,
          headerX,
          78,
          { align: 'right' },
        );
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(10)
        .text('CLIENT: ', 55, infoY + 11, { continued: true })
        .font('Helvetica')
        .text(data.clientName);
      doc
        .fillColor(greyText)
        .fontSize(8)
        .text(
          `Dossier N°: ${data.ficheNumber || data.bcNumber.split('-').pop()}`,
          400,
          infoY + 12,
        ...
        [
          {
            ...
            values: [
              ...
              data.prescription.od.cylindre,
              data.prescription.od.axe,
              data.prescription.od.addition,
            ],
          },
          {
            label: 'OG',
            values: [
              data.prescription.og.sphere,
              data.prescription.og.cylindre,
              data.prescription.og.axe,
              data.prescription.og.addition,
            ],
          },
        ],
        [30, 55, 55, 55, 55],
      ...
      drawTable(
        305,
        currentY,
        ['', 'MARQUE', 'MODÈLE', 'RAYON', 'DIA'],
        [
          {
            label: 'OD',
            values: [
              data.lentilles.od.marque,
              data.lentilles.od.modele,
              data.lentilles.od.rayon,
              data.lentilles.od.diametre,
            ],
          },
          {
            label: 'OG',
            values: [
              data.lentilles.og.marque,
              data.lentilles.og.modele,
              data.lentilles.og.rayon,
              data.lentilles.og.diametre,
            ],
          },
        ],
        [30, 60, 60, 50, 50],
      ...
      drawInfoBox(
        ...
        boxW,
        'Séc. Lacrym (OD/OG)',
        `${data.adaptation.od.secretionLacrimale || '-'}/${data.adaptation.og.secretionLacrimale || '-'} mm`,
      );
      drawInfoBox(
        40 + boxW * 3,
        currentY,
        boxW,
        'B.U.T (OD/OG)',
        `${data.adaptation.od.but || '-'}/${data.adaptation.og.but || '-'} sec`,
      );
      ...
      const drawKRow = (label: string, k: any, y: number) => {
        doc
          ...
          .font('Helvetica-Bold')
          .fontSize(10)
          .text(label, 40, y + 8, { width: kWidth, align: 'center' });
        doc.fillColor(primaryColor).font('Helvetica-Bold').fontSize(10);
        doc.text(k.k1 || '-', 40 + kWidth, y + 8, {
          width: kWidth,
          align: 'center',
        });
        doc.text(k.k2 || '-', 40 + kWidth * 2, y + 8, {
          width: kWidth,
          align: 'center',
        });
        doc.text(`${k.axe || '-'}°`, 40 + kWidth * 3, y + 8, {
          width: kWidth,
          align: 'center',
        });
        doc.text(k.kMoy || '-', 40 + kWidth * 4, y + 8, {
          width: kWidth,
          align: 'center',
        });
        doc
      ...
      doc.text('K1 (H)', 40 + kWidth, currentY, {
        width: kWidth,
        align: 'center',
      });
      doc.text('K2 (V)', 40 + kWidth * 2, currentY, {
        width: kWidth,
        align: 'center',
      });
      doc.text('AXE', 40 + kWidth * 3, currentY, {
        width: kWidth,
        align: 'center',
      });
      doc.text('K.MOY', 40 + kWidth * 4, currentY, {
        width: kWidth,
        align: 'center',
      });
      doc
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fontSize(9)
        .text('OPTICIEN / ADAPTATEUR', 40, sigY + 10, {
          width: 210,
          align: 'center',
        });
      doc.text('CACHET ÉTABLISSEMENT', 345, sigY + 10, {
        width: 210,
        align: 'center',
      });
      ...
      if (data.branding?.cachetUrl) {
        try {
          const base64Cachet = data.branding.cachetUrl.includes('base64,')
            ? data.branding.cachetUrl.split('base64,')[1]
            : data.branding.cachetUrl;
          doc.image(Buffer.from(base64Cachet, 'base64'), 380, sigY + 30, {
            height: 45,
          });
        } catch (e) {
          this.logger.error(
            `Failed to embed lens tech sheet cachet: ${e.message}`,
          );
        }
      }
