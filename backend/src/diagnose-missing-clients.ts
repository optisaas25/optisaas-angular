...
async function main() {
  ...
  console.log(
    `Found ${recentlyCreated.length} clients created today (showing last 20):`,
  );
  recentlyCreated.forEach((c) => {
    console.log(
      `- ID: ${c.id}, Nom: ${c.nom}, Statut: ${c.statut}, CentreId: ${c.centreId}, Date: ${c.dateCreation}`,
    );
  });

  // Check count of INACTIF clients
  const inactifCount = await prisma.client.count({
