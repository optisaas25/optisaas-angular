...
export class AntivirusService {
  ...
  async scanFile(
    filePath: string,
  ): Promise<{ clean: boolean; threat?: string; scanTime?: number }> {
  ...
  async scanFiles(
    filePaths: string[],
  ): Promise<Map<string, { clean: boolean; threat?: string }>> {
    ...
    const cleanCount = Array.from(results.values()).filter(
      (r) => r.clean,
    ).length;
    console.log(
      `[ANTIVIRUS] Batch scan complete: ${cleanCount}/${filePaths.length} files clean`,
    );
