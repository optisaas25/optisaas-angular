...
export class ImportsController {
  ...
  async executeImport(@Body() body: ExecuteImportDto) {
    ...
    console.log(
      'Mapping Keys:',
      body.mapping ? Object.keys(body.mapping) : 'NONE',
    );
    ...
    log(
      `Execute import request START: Type=${body.type}, DataRows=${body.data?.length}`,
    );
    ...
    try {
      if (body.type === 'clients') {
        ...
        const res = await this.importsService.importClients(
          data,
          mapping,
          body.centreId,
        );
        ...
        if (!body.warehouseId) {
          ...
          throw new BadRequestException(
            'Warehouse ID is required for product import',
          );
        }
        const res = await this.importsService.importProducts(
          data,
          mapping,
          body.warehouseId,
        );
        ...
        const res = await this.importsService.importFiches(
          data,
          mapping,
          body.centreId,
          body.type,
        ...
        log(
          `Fiches import completed: ${JSON.stringify({
            success: res.success,
            skipped: res.skipped,
            failed: res.failed,
        ...
        const res = await this.importsService.importFacturesFournisseurs(
          data,
          mapping,
          body.centreId,
          body.isBL,
        ...
        const res = await this.importsService.importPaiementsFournisseurs(
          data,
          mapping,
        );
        ...
        const res = await this.importsService.importFacturesVentes(
          data,
          mapping,
          body.centreId,
        );
        ...
        const res = await this.importsService.importPaiementsClients(
          data,
          mapping,
        );
        ...
        const res = await this.importsService.importDepenses(
          data,
          mapping,
          body.centreId,
        );
      ...
      log(
        `Error: Invalid import type detected: "${body.type}" (Type: ${typeof body.type})`,
      );
