...
import {
  ...
  TypeOperation,
} from './dto/create-operation-caisse.dto';
            ...
            data: {
              ...
              totalVentesEspeces:
                (operation.moyenPaiement === 'ESPECES' || operation.moyenPaiement === 'ESPECE')
                  ...
                  : undefined,
              totalVentesCarte:
                operation.moyenPaiement === 'CARTE'
                  ? { increment: operation.montant }
                  : undefined,
              totalVentesCheque:
                operation.moyenPaiement === 'CHEQUE'
                  ? { increment: operation.montant }
                  : undefined,
            },
          });
          ...
          await tx.journeeCaisse.update({
            ...
            data: {
              ...
              totalTransfertsDepenses:
                operation.motif === 'ALIMENTATION_CAISSE_DEPENSES'
                  ? { increment: operation.montant }
                  : undefined,
            },
          });
        ...
        if (deletedOp.typeOperation === 'COMPTABLE') {
          await tx.journeeCaisse.update({
            ...
            data: {
              ...
              totalVentesEspeces:
                deletedOp.moyenPaiement === 'ESPECES'
                  ? { decrement: deletedOp.montant }
                  : undefined,
              totalVentesCarte:
                deletedOp.moyenPaiement === 'CARTE'
                  ? { decrement: deletedOp.montant }
                  : undefined,
              totalVentesCheque:
                deletedOp.moyenPaiement === 'CHEQUE'
                  ? { decrement: deletedOp.montant }
                  : undefined,
            },
          });
        ...
        if (deletedOp.typeOperation === 'COMPTABLE') {
          ...
          await tx.journeeCaisse.update({
            ...
            data: {
              ...
              totalTransfertsDepenses:
                deletedOp.motif === 'ALIMENTATION_CAISSE_DEPENSES'
                  ? { decrement: deletedOp.montant }
                  : undefined,
            },
          });
