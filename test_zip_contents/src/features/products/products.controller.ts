...
export class ProductsController {
  ...
  update(@Param('id') id: string, @Body() updateProductDto: UpdateProductDto) {
    if ((updateProductDto as any).specificData) {
      ...
      console.warn(
        `   Payload specificData keys: ${Object.keys((updateProductDto as any).specificData)}`,
      );
      console.warn(
        `   PendingOutgoing in payload: ${(updateProductDto as any).specificData.pendingOutgoing ? 'PRESENT' : 'MISSING'}`,
      );
    }
  ...
  initiateTransfer(
    @Param('id') id: string,
    @Body() body: { targetProductId: string; quantite?: number },
  ) {
    return this.productsService.initiateTransfer(
      id,
      body.targetProductId,
      body.quantite !== undefined ? Number(body.quantite) : undefined,
    );
  }
  ...
  restock(
    ...
    return this.productsService.restock(
      ...
      body.remiseFournisseur !== undefined
