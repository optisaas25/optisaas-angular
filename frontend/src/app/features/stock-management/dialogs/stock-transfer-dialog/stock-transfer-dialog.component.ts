...
export class StockTransferDialogComponent implements OnInit {
    ...
    onSourceChange(warehouseId: string): void {
        ...
        if (matchingProduct) {
            ...
            this.productService.findAll({ entrepotId: warehouseId }).subscribe({
                next: (products) => {
                    const targetCode = (this.data.product.codeInterne || '').trim().toLowerCase();
                    const targetBar = (this.data.product.codeBarres || '').trim().toLowerCase();

                    const found = products.find(p => {
                        const pCode = (p.codeInterne || '').trim().toLowerCase();
                        const pBar = (p.codeBarres || '').trim().toLowerCase();

                        return (targetCode && pCode === targetCode) ||
                            (targetBar && pBar === targetBar) ||
                            (p.designation === this.data.product.designation); // Last resort exact name match
                    });

                    console.log('Lookup result for', targetCode, 'in', warehouseId, 'found:', found ? 'Yes' : 'No', 'Count:', products.length);
    ...
    setMatchingProduct(product: any): void {
        if (product) {
            // Removed incorrect check (product.id === this.data.product.id) which blocked valid transfers where source == context
